...
reuse_certificate_placeholder() {
  source ../skyfalconua.github.io/libmyserver.sh &&
  cd templates/nginx && \
  template__nginx_placeholder_crt > placeholder.crt && \
  template__nginx_placeholder_key > placeholder.key
}

generate_certificate_placeholder() {
  cd templates/nginx && openssl req -new \
    -newkey rsa:4096 -days 9999 -nodes -x509 \
    -subj "/C=cc/ST=st/O=oo/CN=placeholder.com" \
